...
class LevelBrigandineArmor
{
    ...
    private void StatusViewRequested(IPlayer player, ref StringBuilder stringBuilder, string levelType)
    {
        ...
        {
            stringBuilder.AppendLine($"{Lang.Get($"levelup:{pair.Value.ToLower()}")}: {player.Entity.WatchedAttributes.GetInt($"LevelUP_Level_BrigandineArmor_Sub_{pair.Value}")}");
        }
    }
    ...
    private void ViewReceived(IPlayer player, ItemSlot armorSlot)
    {
        ...
        LevelBrigandineArmorEvents.ExecuteItemInfoUpdated(armorSlot.Itemstack.Item.GetCollectibleBehavior<CollectibleBehaviorWearable>(true), player);
    }
    ...
    private void DamageReceived(IPlayer player, List<ItemSlot> items, ref float damage)
    {
        ...
        foreach (ItemSlot armorSlot in items)
        {
            ...
            double multiply = Configuration.expMultiplyHitBrigandineArmor[armorSlot.Itemstack.Collectible.Code];
