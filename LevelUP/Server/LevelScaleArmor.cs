...
class LevelScaleArmor
{
    ...
    private void StatusViewRequested(IPlayer player, ref StringBuilder stringBuilder, string levelType)
    {
        ...
        {
            stringBuilder.AppendLine($"{Lang.Get($"levelup:{pair.Value.ToLower()}")}: {player.Entity.WatchedAttributes.GetInt($"LevelUP_Level_ScaleArmor_Sub_{pair.Value}")}");
        }
    }
    ...
    private void ViewReceived(IPlayer player, ItemSlot armorSlot)
    {
        ...
        LevelScaleArmorEvents.ExecuteItemInfoUpdated(armorSlot.Itemstack.Item.GetCollectibleBehavior<CollectibleBehaviorWearable>(true), player);
    }
    ...
    private void DamageReceived(IPlayer player, List<ItemSlot> items, ref float damage)
    {
        ...
        foreach (ItemSlot armorSlot in items)
        {
            ...
            double multiply = Configuration.expMultiplyHitScaleArmor[armorSlot.Itemstack.Collectible.Code];
