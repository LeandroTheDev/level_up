...
class OverwriteBlockBreak
{
    ...
    public static void GetDrops(BlockCrop __instance, IWorldAccessor world, BlockPos pos, IPlayer byPlayer, float dropQuantityMultiplier = 1f)
    {
        // Check if is from the server
        if (byPlayer is IServerPlayer && world.Side == EnumAppSide.Server)
        {
            IServerPlayer player = byPlayer as IServerPlayer;
            // Increasing crop drop rate
            player.Entity.Stats.Set("wildCropDropRate", "wildCropDropRate", Configuration.FarmingGetHarvestMultiplyByEXP(player.Entity.WatchedAttributes.GetAsInt("LevelUP_Farming")));
        }
    }
    ...
    {
        // Check if is server side
        if (__instance.Api is ICoreServerAPI)
        {
    ...
    public static bool DoSmelt(BlockCookingContainer __instance, IWorldAccessor world, ISlotProvider cookingSlotsProvider, ItemSlot inputSlot, ItemSlot outputSlot)
    {
        ...
        if (temporaryCookPlayer != null)
        {
            player = temporaryCookPlayer;
            temporaryCookPlayer = null;
        }
        else return true; // Run the native code instead
